...
tab_atlas, tab_predictions, tab_exports = st.tabs([
...
with tab_atlas:
    ...
    with col_map:
        ...
        map_output = st_folium(m, height=450, use_container_width=True, key="atlas_map")
...
with tab_predictions:
    ...
    with col_layout_right:
        ...
        with col_inner_map:
            ...
            map_pred_output = st_folium(m_pred, height=400, use_container_width=True, key="prediction_map")
...
with tab_exports:
    ...
    with col_d1:
        ...
        st.download_button(label="📥 Stáhnout demografii (CSV)", data=df_exp_dem.to_csv(index=False).encode('utf-8'), file_name=f"demografie_{export_orp}.csv", mime="text/csv")
        
    with col_d2:
        filtered_serv = [s for s in services if s["orp"] == selected_orp] if selected_orp != "Celý kraj" else services
        st.download_button(label="📥 Stáhnout registr služeb (CSV)", data=pd.DataFrame(filtered_serv).to_csv(index=False).encode('utf-8'), file_name=f"sluzby_{selected_orp}.csv", mime="text/csv")
        
    with col_d3:
        st.download_button(label="📥 Stáhnout analýzu deficitů (JSON)", data=json.dumps(predictions_val, ensure_ascii=False, indent=2), file_name=f"predikce_{pred_year}.json", mime="application/json")
